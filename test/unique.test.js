...
describe('Unique', () => {
  ...
  it('should return only unique values', () => {
    ...
    const result2 = helpers._uniqueWithObject(startingList);
    const result3 = helpers._uniqueWithSet(startingList);

    assert.equal(result1.length, 10);
    assert.equal(result2.length, 10);
    assert.equal(result3.length, 10);
